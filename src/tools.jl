...
toSVectors(vs...) where {N} = [promote(toSVector.(vs)...)...]
toSVector(v::SVector) = v
toSVector(v::NTuple{N,Number}) where {N} = SVector(v)
# Dynamic dispatch
toSVector(v::AbstractVector) where {N,M} = SVector(Tuple(v))

ensureSMatrix(m::T) where T<:Number = SMatrix{1,1,T,1}(m)
ensureSMatrix(m::SMatrix) = m
ensureSMatrix(m::Matrix) = throw(ErrorException("Write all model terms using scalars or @SMatrix[matrix]"))
...
@inline padrightbottom(s::SMatrix{E,L}, st::Type{S}) where {E,L,E2,L2,T2,S<:SMatrix{E2,L2,T2}} =
    SMatrix{E2,L2,T2}(ntuple(k -> _padrightbottom((k - 1) % E2 + 1, (k - 1) ÷ E2 + 1, zero(T2), s), Val(E2 * L2)))
...
_collectfirst(ts::NTuple{N,Sublat}, s::Sublat, ss...) where {N} = _collectfirst((ts..., s), ss...)
