...
- `Sublat{T,E}`: a set of sites, given as a `Vector` of positions `SVector{E,T}` (`E` is the embedding dimension, and `T<:Number` is the type of coordinates)
    ...
    - We can establish the number of orbitals and name of this `Sublat` with `Sublat((1.0,0), (0,1)), norbitals = 2, name = :A)`
...
- `Model`: A tight-binding model given as a set of `hopping` and `onsite` instructions `Model(onsite(...), hopping(...), ...)` used to produce the (Bloch) Hamiltonian of a given system. Any unspecified `hopping` or `onsite` in a `Model` will be taken as zero in the resulting Hamiltonian. Hence and empty `Model()` will produce just a Hamiltonian full of zeros. The syntax of `hopping` and `onsite` terms are as follows:
    - `hopping(h, sublats = ((s1, s2), (s3, s4), ...), ndists = (n1, n2, ...)), range = 1)`
    - `onsite(o, sublats = (s1, s2, ...))`
    - Here `h` and `o` are either constants, matrices in orbital space (use e.g. `@SMatrix[1 2; 3 4]` for performance) or functions of position. Onsite functions take one position (`o = r -> ofunc(r)`), and hopping functions take the position `r = (r1 + r2)/2` of the center of the link between `r1` and `r2`, and the link vector `dr = r2 - r1` (`h = (r,dr) -> hfunc(r,dr)`)
...
- `System`: the basic object whose properties we want to compute. Its a combination of `Sublat`s, a `Bravais` matrix and a tight-binding `Model` that becomes translated into a Hamiltonian `Operator`.
    - The canonical constructor is `System(sublats..., bravais, model)`
    - Alternatively we can use presets, defined in the `Dict` `systempresets`. *Example*: `System(:honeycomb, Model(hopping(1)))`
    - We can modify the type of positions `T`, the Hamiltonian element type `Tv` and the embedding dimension `E` using keyword arguments, as in `System(:honeycomb, dim = Val(E), postype = T, hamtype = Tv)`.

## System API

A number of functions are available to easily build complex systems incrementally. These are 
- `modifysublats!(system; names, norbitals)` to change the names and number of orbitals of the different sublattices in a system.
...
One can use a functional for of all the above (except `combine` with more than one `system`) to chain a number of operations. The two following instructions would then be equivalent
```julia
...
System{Complex{Float64},Float64,2,2} : 2D system in 2D space
  ...
  Coordination        : 3.0
```
The resulting object is of type `System{Tv,T,E,L}`, where `Tv = Complex{Float64}` is the type of the Hamiltonian elements, `T = Float64` is the numeric type of positions, `E = 2` is the embedding dimension (2D space in this example), and `L = 2` is the lattice dimension (two because graphene is a 2D Bravais lattice, with two bravais vectors). The reported coordination is the average number of links out of each site (3 in graphene with nearest neighbors). The number `Total hoppings` indicates how many links exist within the unit cell and between the unit cell and other linked unit cells. In this case this is six, see the following sketch.
```
 \/
 ||
 /\
...
```julia
julia> System(:triangular, Model(hopping(@SMatrix[0 2; 0 0])); norbitals = 2)
System{Complex{Float64},Float64,2,2} : 2D system in 2D space
  Bravais vectors     : ((0.5, 0.866025), (-0.5, 0.866025))
  Sublattice names    : (:S1,)
  Sublattice orbitals : (2,)
  Total sites         : 1 [Float64]
  Total hoppings      : 4 [Complex{Float64}]
  Coordination        : 4.0
```
...
```
A few other keyword arguments are available for this particular preset (check `systempresets` in `presets.jl`). In all presets the default System keywords `dim`, `ptype` and `htype` are furthermore available.
...
```julia
sys0 = System(:honeycomb, norbitals = 2)
t1 = 1
t2 = 0.1
σ0 = @SMatrix[1 0; 0 1]
σz = @SMatrix[1 0; 0 -1]
